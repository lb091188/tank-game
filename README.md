...
## 阶段二：WebSocket 内网对战（已上线 M5+M6）

死斗模式（2-8 人 · 3 分钟 · 击杀计分 · 5 秒重生 · 主机权威判定）。

**开战步骤**：
1. 任意一人当房主：`cd server && npm install && npm start`（默认端口 8342，控制台会打印内网 WS 地址）
2. 每个玩家从任意入口打开游戏（房主的 `http://内网IP:8342` 或本地打开均可）→ 标题屏点「⚔ 联机对战」
3. 在联机设置里填房主控制台打印的 **WS 地址**（如 `192.168.1.5:8342`，会记忆）
4. 第一人「创建房间（成为主机）」得到房间号；其他人填房间号加入并「准备」
5. 主机选地图 → 「开始对战」。出击界面选的坦克即联机用车

架构：房主浏览器跑完整模拟与装甲判定（输入 30Hz 上行 / 快照 20Hz 下行 / 120ms 插值）；服务器只做静态托管 + 房间 + 消息转发；命中/击毁/重生/计分全由主机裁决后广播，客户端本地表现（特效/音效/伤害数字/点亮灯泡）。

> 注意：从 https 页面（如 GitHub Pages 在线版）连内网 ws:// 可能被浏览器混合内容策略拦截，建议用房主打印的地址直接打开。
